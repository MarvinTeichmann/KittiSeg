...
def _make_data_gen(hypes, phase, data_dir):
    ...
    for image, gt_image in data:
        ...
        for i in range(100):
            ...
            mygt = np.all(gt_patch == road_color, axis=2)
            ...
            if np.sum(mygt) == 0:
                if random.random() > 0.66:
                    ...
                    yield image_patch, 0
            elif np.sum(mygt) > 0.5 * num_pixels:
                Road += 1
                image_patch = image[x:(x + image_size), y:(y + image_size)]
                yield image_patch, 1
        logging.debug("Road: %i Background: %i Ratio: %s",
                      Road, Background, Road/(Road+Background) )
...
def main():
    ...
    with open('../hypes/kitti.json', 'r') as f:
        hypes = json.load(f)
